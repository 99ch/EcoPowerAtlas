...
class Country(TimeStampedModel):
	...
	population = models.BigIntegerField(null=True, blank=True, validators=[MinValueValidator(0)])
...
class HydroSite(TimeStampedModel):
	...
	head_m = models.DecimalField(max_digits=6, decimal_places=2, null=True, blank=True, validators=[MinValueValidator(0)])
	storage_capacity_mwh = models.DecimalField(max_digits=10, decimal_places=2, null=True, blank=True, validators=[MinValueValidator(0)])
	turbine_capacity_mw = models.DecimalField(max_digits=9, decimal_places=2, null=True, blank=True, validators=[MinValueValidator(0)])
	...
	class Meta:
		ordering = ['country__name', 'name']
		constraints = [
			models.UniqueConstraint(fields=['country', 'name'], name='unique_site_per_country'),
		]
...
class ResourceMetric(TimeStampedModel):
	...
	value = models.DecimalField(max_digits=14, decimal_places=4, validators=[MinValueValidator(0)])
	...
	class Meta:
		...
		constraints = [
			models.UniqueConstraint(
				fields=['dataset', 'country', 'resource_type', 'metric', 'year'],
				name='unique_metric_per_scope',
			),
		]
...
class ClimateSeries(TimeStampedModel):
	...
	class Meta:
		...
		constraints = [
			models.UniqueConstraint(
				fields=['dataset', 'country', 'region', 'site', 'variable'],
				name='unique_climate_series',
			),
		]
...
class ReportAttachment(TimeStampedModel):
	site = models.ForeignKey(HydroSite, related_name='attachments', on_delete=models.CASCADE, null=True, blank=True)
	name = models.CharField(max_length=128)
	document = models.FileField(upload_to='reports/%Y/%m/', blank=True, null=True)
	data = models.FileField(upload_to='reports/%Y/%m/', blank=True, null=True)
	description = models.TextField(blank=True)
